...
#fonction renvoie nos matrices A,B et C de notre rep d'état de la forme X_point_a=AaXa+BaUa, Ya=CaXa	
def state_matrix(q1,q2,w1,w2,tau,tau2,g):
	...
	#A_augmenté
	zero=np.zeros((4,4))
	A_new_part=np.hstack((-C,zero))
	Aa=np.hstack((A,zero))
	Aa=np.vstack((Aa,A_new_part))
	
	#B_augmenté
	Ba=np.vstack((B,np.zeros((4,2))))
	#C_augmenté
	Ca=np.hstack((C,np.zeros((4,4))))
	return Aa,Ba,Ca
